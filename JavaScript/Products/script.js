...
const getAllProducts = async (search) => {
    ...
    const api = get_api(endPoint); // example: https://dummyjson.com/products?skip=85
    const response = await request(api);
    console.log(response)
    return response; // return only products array
}
...
const handleProductList = (search) => {
    getAllProducts(search).then(responseData => {
        const products = responseData.products;
        const totalProducts = responseData.total;
        ...
        console.log(loadedProducts.length, totalProducts)
        if (loadedProducts.length === totalProducts) {
            loadMore.style.display = "none";
        }
        products.forEach(product => {
            if (!loadedProducts.includes(product.id)) { // if product already loaded, return
                loadedProducts.push(product.id); // add product id to loadedProducts array
...
window.onload = handleProductList();
...
const handleSearch = search => {
    ...
    handleProductList(search);
}

const handleLoadMore = () => {
    const search = document.getElementById("search-input");
    search.oninput = () => isFirstSearch = true;
    const value = search.value;
    if (!value) return handleProductList();
...
searchButton.addEventListener("click", handleLoadMore);